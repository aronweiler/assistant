...
MULTI_DESTINATION_ROUTER_TEMPLATE = """<s>[INST] <<SYS>>
SYSTEM INFORMATION:
{{system_information}}
...
REMEMBER: "next_inputs" should just be the original input.
...
OUTPUT (JSON object matching the specified format):[/INST]
"""

CONVERSATIONAL_TEMPLATE = """<s>[INST] <<SYS>>{system_prompt}
System information:
{system_information}
Loaded documents:
{loaded_documents}
...
Below is a conversation between a user and an AI.  Respond to the most current message in the conversation.

Current conversation:
{chat_history}<</SYS>>
Current message:
{user_name} ({user_email}): {input}[/INST]"""
