...
class InteractionManager:
    ...
    def _ensure_interaction_exists(self, user_id: int):
        ...
        with self.interactions_helper.session_context(
            ...
            if interaction is None:
                ...
                logging.info(f"Interaction ID: {self.interaction_id} created for user {user_id}")
            else:                
                # The interaction already exists, but could still need summary
                self.interaction_needs_summary = interaction.needs_summary

                logging.info(f"Interaction ID: {self.interaction_id} already exists for user {user_id}, needs summary: {self.interaction_needs_summary}")
    ...
    def _create_conversation_memory(self, llm, max_token_limit):
        ...
        logging.info(f"Conversation memory created for interaction {self.interaction_id}, with max token limit {max_token_limit}")
