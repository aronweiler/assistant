...
class LLMThought:
    ...
    def __init__(
        self,
        llm_container: DeltaGenerator,
        agent_container: DeltaGenerator,
        labeler: LLMThoughtLabeler,
        expanded: bool,
        collapse_on_complete: bool,
    ):
        self._llm_container = llm_container
        self._agent_container = agent_container.status(
        ...
        self.in_tool_action = False

    @property
    def llm_container(self) -> "StatusContainer":
        """The container we're writing into."""
        return self._llm_container
    
    @property
    def agent_container(self) -> "StatusContainer":
        """The container we're writing into."""
    ...
    def _reset_llm_token_stream(self) -> None:
        # if self._llm_container is not None:
        #     self._llm_container.markdown(self._llm_token_stream)

        # self._llm_token_stream = ""
        # self._llm_container = None
        pass
    ...
    def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        if self.in_tool_action:
            self._llm_token_stream += _convert_newlines(token)
            if self._agent_container is None:
                raise RuntimeError(
                    "Missing an Agent container"
                )
            
            self._agent_container.markdown(self._llm_token_stream + "▕")
        else:
            self._llm_token_stream += _convert_newlines(token)
            if self._llm_container is None:
                raise RuntimeError(
                    "Missing an LLM container"
                )
    ...
    def on_llm_error(
        ...
        self._llm_container.markdown("**LLM encountered an error...**")
        self._agent_container.exception(error)
    ...
    def on_tool_start(
        ...
        self.in_tool_action = True
        self._state = LLMThoughtState.RUNNING_TOOL
        tool_name = serialized["name"]
        self._last_tool = ToolRecord(name=tool_name, input_str=input_str)
        self._agent_container.update(
    ...
    def on_tool_end(
        ...
        **kwargs: Any,
        ...
        self._agent_container.markdown(output)
        self.in_tool_action = False

    def on_tool_error(
        self, error: Union[Exception, KeyboardInterrupt], **kwargs: Any
    ) -> None:
        self._llm_container.markdown("**Tool encountered an error...**")
        self._agent_container.exception(error)
        self._agent_container(state="error")
        self.in_tool_action = False
...
class StreamlitCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(
        self,
        llm_parent_container: DeltaGenerator,
        agent_parent_container: DeltaGenerator,
        *,
        ...
        """
        self._llm_parent_container = llm_parent_container
        self._agent_parent_container = agent_parent_container
    ...
    def on_llm_start(
        ...
        if self._current_thought is None:
            self._current_thought = LLMThought(
                llm_container=self._llm_parent_container,
                agent_container=self._agent_parent_container,
