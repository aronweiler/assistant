...
_DEFAULT_WORTHINESS_TEMPLATE = """You are an AI assistant reading the transcript of a conversation between an AI and a human, and determining if the last line of conversation contains certain types of information worthy of storing in your memory for later use.

Examine the conversation history and the last line of conversation.  If the last line of conversation is not information that we should store in memory (i.e. there is no value that we could later derive by storing it now- for example: a user asking a question is generally not considered worthy, however if their question includes information about people, places, or things of interest, then it is worthy), respond with 'NOT WORTHY'.  If the last line of conversation is information that we should store in memory (i.e. there is some value that we could later derive by storing it now), respond with 'WORTHY'.  If you are unsure, respond with 'WORTHY'.

Things that have value are things that are relevant to the subject of the conversation, or things that are relevant to the user's interests.  For example, if the user is talking about a specific person, then the name of that person is relevant to the current conversation.  If the user is talking about a specific person, and that person is a friend of the user, then the name of that person is relevant to the user's interests. Places that the user mentions are always relevant to the user's interests.
...
_DEFAULT_ENTITY_EXTRACTION_TEMPLATE = """You are an AI assistant reading the transcript of a conversation between an AI and a human. Extract all of the nouns, proper nouns and pronouns from the last line of conversation. You should always extract all places and people.  Extract things if they are relevant to the subject of the conversation.  Always extract possessive nouns, such as "car" in the phrase "my car".

The conversation history is provided just in case of a coreference (e.g. "What do you know about her/him/them" where "her/him/them" is defined in a previous line) -- ignore items mentioned there that are not in the last line for extraction.

Return the output as a single comma-separated list, or NONE if there is nothing of note to return (e.g. the user is just issuing a greeting, asking a question, or engaging in simple conversation).

EXAMPLE
Conversation history:
Jim: how's it going today?
AI: "It's going great! How about you?"
Jim: good! busy working on Langchain. lots to do.
AI: "That sounds like a lot of work! What kind of things are you doing to make Langchain better?"
Last line:
Jim: I'm trying to improve Langchain's interfaces, the UX, its integrations with various products the user might want... a lot of stuff.
Output: Jim, Langchain
END OF EXAMPLE
...
AI: "That sounds like a lot of work! What kind of things are you doing to make Langchain better?"
Last line:
Jim: I'm trying to improve Langchain's interfaces, the UX, its integrations with various products the user might want ... a lot of stuff. I'm working with Alex.
Output: Jim, Langchain, Alex
END OF EXAMPLE
...
_RELATED_ENTITY_TEMPLATE = """You are an AI assistant helping a human keep track of facts about relevant people, places, and things in their life. 

Look at the existing summary, and then read the last line of conversation with {human_prefix}. Determine whether the last line of conversation contains any new information about the provided summary. If so, respond only with 'RELATED' and nothing else. If not, respond only with 'UNRELATED' and nothing else.  If the last line of the conversation is related to the provided summary, but does not contain any information that is not already captured by the summary, respond only with 'NONE' and nothing else.
...
_DEFAULT_ENTITY_SUMMARIZATION_TEMPLATE = """You are an AI assistant helping a human keep track of facts about relevant people, places, and things in their life. 
...
The update should only add facts to the original summary that are relayed in the last line of conversation about the given entity to summarize.
