...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			# check dir permissions to reduce likelihood of half-initialized database, ignore if this call fails            
			cmd_status=$(ls /docker-entrypoint-initdb.d/ > /dev/null 2>&1)
            if [ $? -eq 0 ]; then
                echo "'ls /docker-entrypoint-initdb.d/' command succeeded."
            else
                echo "'ls /docker-entrypoint-initdb.d/' command failed. "
            fi
