...
class SpreadsheetsTool:
    ...
    def query_spreadsheet_pandas(self, query: str, target_file_id: int):
        ...
        agent = self.create_pandas_agent(
            llm=self.llm,
            files=[file]            
        )
    ...
    def create_pandas_agent(
        ...
        pandas_agent = create_pandas_dataframe_agent(llm=llm, df=dfs, include_df_in_prompt=True, number_of_head_rows=3, verbose=True, callbacks=self.callbacks)        
        ...
        for file in files:
            if not file.id in self.cached_pandas_dataframes:
                ...
                if file_extension in self.excel_types:                    
                    ...
                    self.cached_pandas_dataframes[file.id].append(pd.read_csv(filepath_or_buffer=reader, on_bad_lines='skip', encoding='ISO-8859-1', **kwargs))                    
            else:
                logging.info(f"File {file.file_name} already in pandas cache, skipping.")
