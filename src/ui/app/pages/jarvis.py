...
if __name__ == "__main__":
    ...
    try:
        ...
        conversations_tab, document_tab, file_uploads_tab = st.sidebar.tabs(
            ["Conversations", "Documents", "File Uploads"]
        )

        # Time the operation
        start_time = time.time()
        ui_shared.load_conversation_selectbox(rag_ui.load_ai, conversations_tab)
        ...
        ui_shared.select_documents(
            ai=st.session_state["rag_ai"], tab=document_tab
        )
        logging.info(f"Time to show select documents: {time.time() - start_time}")
        
        # Time the operation
        start_time = time.time()
        ui_shared.file_uploads(
            ai=st.session_state["rag_ai"], tab=file_uploads_tab
        )
        logging.info(f"Time to show file_uploads: {time.time() - start_time}")

        # Time the operation
        start_time = time.time()
        ui_shared.handle_chat(st.session_state["rag_ai"])
