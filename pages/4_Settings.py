...
def settings_page():
    ...
    # Source Code URL
    source_code_url = st.text_input("Source Code URL", os.getenv("SOURCE_CODE_URL"))

    # Source Code Personal Access Token (PAT)
    pat = st.text_input("Source Code Personal Access Token (PAT)", type="password", value=os.getenv("PAT"))
    ...
    if st.button("Save Settings"):
        ...
        if source_code_url:
            os.environ["SOURCE_CODE_URL"] = source_code_url
        ...
        print(f"SOURCE_CODE_URL: {os.getenv('SOURCE_CODE_URL', 'NOT SET')}")
        print(f"PAT: {os.getenv('PAT', 'NOT SET')}")
